...
def test_create_order_empty_items_returns_400(client):
    ...
    response = client.post("/orders", json={"items": []}, headers=buyer_headers)
    assert response.status_code == 400
...
def test_list_my_orders_only_returns_own_orders(client, db_session):
    ...
    client.post("/orders", json={"items": [{"product_id": product.id, "quantity": 1}]}, headers=buyer_a_headers)
...
def test_get_order_detail_forbidden_for_other_user(client, db_session):
    ...
    create_response = client.post(
        "/orders", json={"items": [{"product_id": product.id, "quantity": 1}]}, headers=owner_headers
    )
